...
// Parse raw tree object data into a Tree struct.
int tree_parse(const void *data, size_t len, Tree *tree_out);

// Serialize a Tree struct into raw bytes for object_write(OBJ_TREE, ...).
// Entries MUST be sorted by name before serialization.
// Caller must free(*data_out).
int tree_serialize(const Tree *tree, void **data_out, size_t *len_out);

// Build a Tree from the current index contents.
// Handles nested paths (e.g., "src/main.c") by building subtrees.
